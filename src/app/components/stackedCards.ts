...
export const subject = new BehaviorSubject(0); // 0 is the initial value
...
export default function stackedCards() {
    ...
    const elementsMargin = 5; // Define the distance of each element when the stacked options are bottom or top.
    ...
    for (let i = items; i < maxElements; i++) {
        ...
        listElNodesObj[i].style.webkitTransform =
            'scale(' + (1 - (items * 0.04)) + ') translateX(0) translateY(' + elTrans + 'px) translateZ(0)';
    ...
    function changeStages() {
        if (currentPosition === maxElements) {
            ...
            listElNodesObj[maxElements - 1].addEventListener('transitionend', () => {
    ...
    //  onSwipeRight and onSwipeTop and put the method just above the letiable 'currentPosition = currentPosition + 1'.
    // On the actions onSwipeLeft, onSwipeRight and onSwipeTop you need to remove the currentPosition letiable
            ...
            if (stackedOptions === 'Top') {
                ...
                if (element) {
                    element.style.webkitTransform =
                        'translateX(' + moveX + 'px) translateY(' + (moveY + elTrans) + 'px) translateZ(0) rotate(' + rotateElement + 'deg)';
                ...
                if (element) {
                    element.style.webkitTransform = 'translateX(' + moveX + 'px) translateY(' + (moveY) + 'px) ' +
                        'translateZ(0) rotate(' + rotateElement + 'deg)';
                ...
                if (listElNodesObj[i]) {
                    if (stackedOptions === 'Top') {
                        ...
                        listElNodesObj[i].classList.add(
                            'stackedcards-bottom', 'stackedcards--animatable', 'stackedcards-origin-bottom'
                        );
                    ...
                    listElNodesObj[i].style.transform = 'scale(' + elScale + ') ' +
                        'translateX(0) translateY(' + (elTrans - elTransInc) + 'px) translateZ(0)';
                    listElNodesObj[i].style.webkitTransform = 'scale(' + elScale + ') translateX(0)' +
                        'translateY(' + (elTrans - elTransInc) + 'px) translateZ(0)';
